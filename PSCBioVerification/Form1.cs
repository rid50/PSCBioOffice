...
/*
            string text2 = String.Format("Template extracted{0}. G: {1}. Size: {2}", true,
                Data.NFExtractor.UseQuality ? string.Format(". Quality: {0:P0}", Utils.QualityToPercent(template.Quality) / 100.0) : null,
                template.G, Data.SizeToString(template.Save().Length));
*/
            ...
            /*
                        if (availableCount == count)
                        {
                            ...
                            lblWaitingForImg.Text = "Waiting for image(s)";

                            if (enrollMode && settings.FPUseGeneralization)
                            {
                                ...
                                try
                                {
                                    template = Data.NFExtractor.Generalize(templates, out extractionStatus, out baseTemplateIndex);
                                }
                                catch (Exception e)
                                {
                                    string text = string.Format("Generalization error: {0}", e.Message);
                                    LogLine(text, true);
                                    MessageBox.Show(text, Text, MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    return;
                                }
                                ...
                                if (extractionStatus != NfeExtractionStatus.TemplateCreated)
                                {
                                    string text = "Generalization failed: " + extractionStatus.ToString();
                                    LogLine(text, true);
                                    MessageBox.Show(text, Text, MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    return;
                                }
                                LogLine("Template generalized, based on image #{0}{1}. G: {2}. Size: {3}. Time {4}",
                                    true, baseTemplateIndex + 1,
                                    Data.NFExtractor.UseQuality ? string.Format(". Quality: {0:P0}", Utils.QualityToPercent(template.Quality) / 100.0) : null,
                                    template.G, Data.SizeToString(template.Save().Length), Data.TimeToString(swg.Elapsed));
                                nfView1.Image = bitmaps[baseTemplateIndex];
                                nfView2.Image = bitmaps[baseTemplateIndex];
                                nfView2.ResultImage = resultBitmaps[baseTemplateIndex];
                                nfView2.Template.Dispose();
                                nfView2.Template = template;
                                this.image = images[baseTemplateIndex];
                                this.resultImage = resultImages[baseTemplateIndex];
                            }
            */
            switch (mode)
            {
...
/*
            ...
            WsqImage wsqImage = _fingersCollection[fingerNumber] as WsqImage;
